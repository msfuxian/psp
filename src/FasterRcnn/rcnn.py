...
class Rcnn(nn.Cell):
    ...
    def construct(self, featuremap, bbox_targets, labels, mask):
        ...
        if self.proto:
            ...
            bg_proto = ms.tensor.from_numpy(prototype).mean(0, True)
            prototype = msnp.concatenate((prototype, bg_proto), 0)
            proto_w = self.proto_mlp(prototype)
        ...
        if self.training:
            if self.ranking:
                ...
                x_cls = msnp.concatenate([x_cls[:, :-1], bg_cls], axis=1)
